...
const continuousRequestHistory = {};
const allRequestHistory = {};
let beforeRequestUrl = '';
/**
...
function compareJSON(expected, actual) {
  try {
    if (typeof expected === 'object') {
      ...
      } else if (Object.keys(expected).length !== Object.keys(actual).length) {
        return false;
      }
      for (let k in expected) {
        ...
        if (!compareJSON(expected[k], actual[k])) {
          return false;
        }
      }
      ...
      if (expected !== '#ANY_VALUE#' && expected !== actual) {
        return false;
      }
    ...
    console.log(`Error during validation: ${exception} ${expected}, ${actual}`);
    return false;
  }
  return true;
}
...
const mockserver = {
  ...
  init: function (directory, verbose, callback) {
    if (this.directory) {
      this.backup = this.directory;
    }
    this.directory = directory;
    this.verbose = !!verbose;
    this.headers = prepareWatchedHeaders();
    if (!this.requestProcess) {
      this.requestProcess = callback;
    }
  },
  handle: function (req, res) {
    if (!req.url.startsWith('/restapi') && mockserver.backup == null) {
      return mockserver.requestProcess(req, res)
    } else {
      mockserver.backup = null;
    }
    getBody(req, function (body) {
      ...
      if (matched.content) {
        ...
        const reqKey = `${req.url}_${req.method}`
        if (beforeRequestUrl !== reqKey) {
          continuousRequestHistory[reqKey] = 1;
        } else if (reqKey in continuousRequestHistory) {
            continuousRequestHistory[reqKey] += 1;
        }
        beforeRequestUrl = reqKey;
        if (!allRequestHistory[reqKey]) {
          allRequestHistory[reqKey] = 1;
        } else {
          allRequestHistory[reqKey] += 1;
        }
        const dataPath = join(mockserver.directory, path, 'data.yaml');
        if (body.length > 0 && fs.existsSync(dataPath) && fs.lstatSync(dataPath).isFile()) {
          const mockData = yaml.load(fs.readFileSync(dataPath, 'utf8'));
          let jsonBody = JSON.parse(body);
          for (let index in mockData.data) {
            let md = mockData.data[index];
            if (compareJSON(md.request, jsonBody) && ((!md.request_times && !md.request_all_times) || (md.request_times && md.request_times.includes(continuousRequestHistory[reqKey])) || (md.request_all_times && md.request_all_times.includes(allRequestHistory[reqKey])))) {
              mock.body = JSON.stringify(md.response);
              console.log(
                'Reading from ' + `${dataPath.yellow}/[${index}]/[${continuousRequestHistory[reqKey]}]`.yellow + ' file: ' + 'Matched'.green
              );
              if (md.request_times && md.request_count_end && md.request_times.includes(continuousRequestHistory[reqKey])) {
                continuousRequestHistory[reqKey] = 0
              }
              if (md.request_all_times && md.request_count_end && md.request_all_times.includes(allRequestHistory[reqKey])) {
                allRequestHistory[reqKey] = 0
              }
              break;
            }
          }
...
module.exports = function (directory, silent, callback) {
  mockserver.init(directory, silent, callback);
